...
"""
TE Attention Benchmark - Fixed Dtype Issues
"""
...
def main():
    print("=== TE Attention Test - Fixed Dtypes ===")
    ...
    try:
        with te.fp8_autocast(enabled=True, fp8_recipe=fp8_recipe):
            ...
            print("\n--- Testing Real TE Attention (with params_dtype=bfloat16) ---")
            try:
            ...
            if real_te_time != float('inf') and hybrid_time != float('inf'):
                ...
                if speedup > 1.05:
                    print(f"\n🎯 Real TE is {speedup:.3f}x FASTER than hybrid approach")
                    print("✅ CONFIRMED: TE native attention beats cuDNN SDPA!")
                elif speedup < 0.95:
                    print(f"\n📈 Hybrid is {1/speedup:.3f}x FASTER than Real TE")
                    print("⚠️  Unexpected: Hybrid beats native TE")
                else:
                    print(f"\n⚖️  Similar performance: {speedup:.3f}x")
            elif real_te_time != float('inf'):
                print(f"✅ Real TE Attention works: {real_te_time:.3f} ms")
                print("❌ Hybrid approach failed")
            elif hybrid_time != float('inf'):
                print(f"❌ Real TE Attention failed")
                print(f"✅ Hybrid approach works: {hybrid_time:.3f} ms")
            else:
